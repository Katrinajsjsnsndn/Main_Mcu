...
{	
	...
	int i=400;
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_9, (GPIO_PinState)BitValue);
	while(i--);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	int i=400;
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_11, (GPIO_PinState)BitValue);
	while(i--);
	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/
	//...
}
uint8_t MyI2C_R_SDA(void)
{
	uint8_t BitValue;
		int i=400;
	BitValue = HAL_GPIO_ReadPin(GPIOE, (GPIO_PinState)GPIO_PIN_11);
